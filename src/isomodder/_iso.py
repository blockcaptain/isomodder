...
from typing import IO, BinaryIO, Callable, ContextManager, Optional, Type, cast
...
class IsoFile(ContextManager[None]):
    ...
    def write_text(self, path: Path, content: str, file_mode: int = None) -> None:
        with self.open_file_write(path, file_mode=file_mode) as file:
            file.write(content.encode())

    def write_fp(self, path: Path, fp: IO[bytes], length: int, file_mode: int = None) -> None:
    ...
    def replace_text(self, path: Path, content: str) -> None:
        with self.replace_file_write(path) as file:
            file.write(content.encode())

    def open_file_read(self, path: Path) -> BinaryIO:
    ...
    def replace_file_write(self, path: Path) -> ContextManager[BinaryIO]:
        rr_path = make_rr_path(path)
